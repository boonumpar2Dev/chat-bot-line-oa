...
async function callAI(prompt: string, model: string) {
  ...
  try { return JSON.parse(txt); } catch { return { answer: txt, confidence: 80, image_titles: [] }; }
}
...
- 📸 ทุกครั้งที่พูดถึง/แนะนำแพ็กเกจใด ต้องใส่ "แพ็กเกจ: <ชื่อ>" ลงใน image_titles เพื่อส่งรูปพื้นฐานของแพ็กเกจนั้นเสมอ (ถ้ามีรูป tier เจาะจง ให้ใช้ชื่อ tier เต็มแทน)${strictSection}
