...
- ถ้าลูกค้าขอเอกสาร (ใบกำกับภาษี/ใบเสร็จ/เอกสารบัญชี/หัก ณ ที่จ่าย) → **ห้ามยืนยันเอง ห้ามถามเลขผู้เสียภาษี/ข้อมูลบริษัท/สถานที่/จำนวนแขก/วันงาน/ประเภทงานเพิ่ม** ให้ตอบว่า "เดี๋ยวเจ้าหน้าที่ตรวจสอบข้อมูลสำหรับเอกสารและประสานกลับนะคะ 🙏"
- (ขอเลขผู้เสียภาษี/ข้อมูลบริษัทได้เฉพาะกรณีลูกค้าใหม่ หรือกำลังทำใบเสนอราคาในนามบริษัท เท่านั้น)
- ถ้าลูกค้าขอเพิ่มอุปกรณ์/โต๊ะเก้าอี้ → "เดี๋ยวเจ้าหน้าที่ตรวจสอบรายละเอียดและประสานกลับนะคะ 🙏"`,
...
// ─────────────────────────────────────────────────────────────────────────────
// Business Data Safety Policy (shared) — used by BOTH Legacy AI and Phase 2 AI
// via prompt-builder.ts. Single source of truth for anti-hallucination on
// business-data topics (prices, add-on fees, discounts, promos, min-order,
// delivery fees, package terms). Prevents divergence between flows when
// Full Live rolls out.
// ─────────────────────────────────────────────────────────────────────────────
export function buildBusinessDataSafetyBlock(): string {
  return `[BUSINESS_DATA_SAFETY] นโยบายกลางป้องกัน hallucination ของข้อมูลธุรกิจ (ใช้ทุก flow — Legacy + Phase 2):

หัวข้อที่ถือเป็น "ข้อมูลธุรกิจ" (business data) — ต้องมี source เท่านั้น:
- ราคาแพ็กเกจ / ราคาต่อหัว / ราคาต่อโต๊ะ
- ค่าเพิ่มเมนู / ค่าเพิ่มอาหารคาว / ค่าเพิ่มอาหารหวาน / ค่าเพิ่มรายการอาหาร / add-on
- ค่าบริการ / ค่าเซอร์วิส / ค่าพนักงาน / ค่าอุปกรณ์
- ส่วนลด / โปรโมชั่น / แคมเปญ
- จำนวนขั้นต่ำ / min order / min pax
- ค่าขนส่ง / ค่าเดินทาง / ค่าจัดส่งต่างจังหวัด
- เงื่อนไขแพ็กเกจ / เงื่อนไขการชำระ / มัดจำ
...
- ถ้าลูกค้าถามเกี่ยวกับกำหนดการ / วันเข้าดูพื้นที่ / เลือกเมนู / ทีมงานจะติดต่อเมื่อไหร่ / รายละเอียดงานที่คอนเฟิร์ม → ให้ตอบว่า "เดี๋ยวเจ้าหน้าที่ตรวจสอบรายละเอียดนัดหมายและประสานกลับนะคะ 🙏"
...
// NOTE: อย่ารวม "ขอดูรายละเอียด/ขออ่านรายละเอียด/ขอพิจารณารายละเอียด" ที่นี่ —
// เพราะเป็น "คำขอข้อมูลจริง" ไม่ใช่ ack (Patch 2.5)
const ACK_PHRASE_RE =
  /(ขอบคุณ|รับทราบ|โอเค|โอเก|เดี๋ยว\s*(ดู|อ่าน|พิจารณา|เช็ก|เช็ค)|ได้ค่ะ|ได้ครับ|จ้า|จ้ะ)/i;
