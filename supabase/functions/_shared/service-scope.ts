...
export function buildServiceScopeLockPrompt(scope: ServiceScope): string {
  if (scope === "food_only_buffet") {
    return [
      ...
      "- ถ้าลูกค้าถามเรื่อง แพ็กเกจ / ราคา / รายละเอียด / \"มีแบบไหนบ้าง\" / อาหารอย่างเดียว / จัดเลี้ยงนอกสถานที่ / เมนู / รายการอาหาร → **เสนอแพ็ก food-only ที่มีอยู่ใน context ด้านล่างทันที** ไม่ต้องรอถามวันงาน/จำนวนแขก/สถานที่ก่อน",
      "- ถ้า context มี food-only package พร้อมรูป → ระบุ imageTitles ให้ตรงชื่อ เช่น `[\"แพ็กเกจ: งานอาหารเท่านั้น\"]` เพื่อให้ระบบส่งรูปแพ็กให้ลูกค้า",
